...
class App extends Component {
	...
	onSearchChange = keyEvent => {
		...
		let filteredProductDataBySearchField = productData.filter(
				product => product.name.toLowerCase().includes(keyEvent.target.value.toLowerCase())
		);
		this.setState({
	...
	onFurnitureStyleSelectChange = keyEvent => {
		...
		if (keyEvent !== null) {
			...
			productDataFilteredByFurnitureStyleSelected = productData.filter((product) =>
					product.furniture_style.some((furnitureStyle) => furnitureStyleOptionSelectedValSet.has(furnitureStyle))
			);
	...
	render() {
		...
		const deliveryTimeListSorted = App.sortUnique(productData.map(products => products.delivery_time));
		...
		const furnitureStyleListMergedSorted = App.sortUnique(furnitureStyleListMerged.sort());
